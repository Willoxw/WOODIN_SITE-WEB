...
$stmt = db()->prepare("SELECT id, invoice_token FROM orders WHERE customer_id = ? AND status <> 'Annulée'"); $stmt->execute([currentCustomer()['id']]); $orders = $stmt->fetchAll();
$zipPath = tempnam(sys_get_temp_dir(), 'woodin_'); $zip = new ZipArchive(); $zip->open($zipPath, ZipArchive::OVERWRITE);
$added = 0; foreach ($orders as $order) { $path = $order['invoice_token'] ? dirname(__DIR__) . '/invoices/facture_' . $order['invoice_token'] . '.pdf' : ''; if (is_file($path)) { $zip->addFile($path, 'facture_' . (int)$order['id'] . '.pdf'); $added++; } }
